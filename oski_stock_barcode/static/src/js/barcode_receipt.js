...
export class BarcodeReceipt extends Component {
    static template = "oski_stock_barcode.BarcodeReceipt";
    ...
    async _onBarcodeScan(ev) {
        ...
        const result = await rpc('/oski_stock_barcode/scan_product', {
            barcode, picking_id: this.props.pickingId,
        });
        if (!result.found) {
        ...
        if (line) {
            ...
            if (line.done_qty >= line.demand_qty) {
                this.notification.add(`${result.name} — complet !`, { type: 'success' });
            }
        } else {
            this.state.lastScanStatus = 'error';
            ScanFeedback.warning();
            this.notification.add(`${result.name} — pas dans ce bon`, { type: 'warning' });
        }
    }
    ...
    _onProductSelected(product) {
        ...
        if (line) {
            ...
            this.notification.add(`${product.name} — pas dans ce bon`, { type: 'warning' });
        }
    }
    ...
    async _updateQty(productId, qty) {
        await rpc('/oski_stock_barcode/update_line_qty', {
            picking_id: this.props.pickingId, product_id: productId, qty,
        });
    }

    _lineClass(line) {
        if (line.done_qty >= line.demand_qty && line.demand_qty > 0) return 'o_line_complete';
        if (line.done_qty > line.demand_qty) return 'o_line_over';
        return '';
    }
